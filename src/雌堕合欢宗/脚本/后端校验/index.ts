...
import { exposeRuntimeGlobal, requireRuntimeGlobal, waitForRuntimeGlobal } from './runtimeGlobals';

async function bootstrapBackendValidate() {
  const RuntimeMvu = await waitForRuntimeGlobal<any>('Mvu');
  const RuntimeEventOn = requireRuntimeGlobal<any>('eventOn');
  const RuntimeTavernEvents = requireRuntimeGlobal<any>('tavern_events');
  const RuntimeInjectPrompts = requireRuntimeGlobal<any>('injectPrompts');
  const RuntimeUninjectPrompts = requireRuntimeGlobal<any>('uninjectPrompts');

  exposeRuntimeGlobal('Mvu', RuntimeMvu);

  const narrativePromptRuntime = createNarrativePromptRuntime({
    getStatData: () =>
      findStatDataForNarrativePrompt(message_id => RuntimeMvu.getMvuData({ type: 'message', message_id })?.stat_data),
  ...
  RuntimeEventOn(RuntimeTavernEvents.GENERATE_BEFORE_COMBINE_PROMPTS, () =>
  ...
  RuntimeEventOn(RuntimeMvu.events.COMMAND_PARSED, (...args: any[]) => {
  ...
  if (typeof window !== 'undefined') {
    ...
    exposeRuntimeGlobal('__TEST_applyValidatedUpdate', async function (
...
void bootstrapBackendValidate().catch(error => {
  console.error('[hehuan backend_validate] bootstrap failed', error);
});
