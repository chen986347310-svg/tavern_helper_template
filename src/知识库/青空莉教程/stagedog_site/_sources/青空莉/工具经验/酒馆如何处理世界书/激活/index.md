...
- 蓝灯条目 A 被激活, 它的提示词`这是一段文本`将会加入到欲扫描文本中
- 欲扫描文本中没有`这是一段文本`, 绿灯条目 B 没被激活

第二轮激活时:

- 欲扫描文本中现在有`这是一段文本`了, 绿灯条目 B 被激活, 它的提示词也加入到欲扫描文本中

这样的激活尝试会不断进行, **直到有一轮不再有条目被激活**时才结束. 这就是整个递归扫描.
...
: - 可以编写 `<%_ if (条件) { _%>` 来控制一部分提示词是否发送. 例如:
  ...
  - 提供的 `<%= await getwi("世界书名称", "条目名称") >` 将会被替换为对应的条目文本. 这实际不涉及条目本身的激活, 因此无所谓条目是否被启用
