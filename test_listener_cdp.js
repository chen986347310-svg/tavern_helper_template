...
const script = `(async()=>{
  try {
    var mvu = typeof Mvu;
    var result = 'Mvu type: ' + mvu;
    if (mvu !== 'undefined') {
      eventOn(Mvu.events.BEFORE_MESSAGE_UPDATE, function(data) {
        if (data && data.message_content && typeof data.message_content === 'string' && !data.message_content.includes('StatusPlaceHolderImpl')) {
...
client.on('message', (data) => {
  ...
  if (msg.id === 1) {
    ...
    console.log('Result:', val !== undefined ? val : JSON.stringify(msg.result));
    client.close();
    process.exit(0);
  }
});
