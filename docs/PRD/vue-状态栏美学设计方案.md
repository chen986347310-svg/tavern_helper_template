...
本文件早期章节记录了“金册玉牒”与日/夜主题的探索过程；当前 v4 验收以本节为准：

- **主视觉**：以“阴阳双生”双主题为最终方向，优先使用 `chenxiang`（脂白沉香）与 `taohua`（玉骨桃花）变量体系承载色彩、辉光、分割线和字体气质。
- **主界面**：主页状态栏升级为“五气朝元阵”，时辰、灵石、场景、NPC 太极/命轮共同构成横向气运界面。
- **角色状态**：不直接展示裸数值，使用 `灵犀` / `蚀心` 状态语与双环命轮表达好感度、攻略值。
- **心音显影**：心里话只在 `soul_whisper.is_revealed === true` 且 `text` 非空时显示；前端不得伪造 AI 尚未回流的剧情内容。
- **开放场景锚点**：`系统.当前场景` 为开放字符串；`莲灯前苑`、`醉玉小筑`、`绮梦幽阁` 仅作为快捷入口、默认文案与旧数据回退。临时场景必须依赖 `系统.场景上下文` 提供公开度、在场 NPC 与故事钩子。
- **遗留说明**：后文出现“日/夜主题”“金册玉牒”等表述时，视为历史实现记录，不再作为新开发的主设计方向。
...
## 历史实施状态（2026-05-20，日/夜主题阶段，仅作追溯）

### 已完成

- [x] 阶段一: SCSS 变量/Mixin/全局样式创建
- [x] 阶段二: 首页 NpcCard 样式 + 金箔贴片效果
- [x] SystemBar SVG 图标替换（太极/樱花/太阳月亮）
- [x] NpcCard 展开区域进度条重设计
- [x] 装备按钮加大 + 文字颜色修复
- [x] 日/夜主题切换系统（useTheme + CSS 自定义属性，v4 后作为历史实现基础，不作为新设计命名）
- [x] mixin 硬编码色覆写（html[data-theme=light] 块）
- [x] 日间模式字体对比度优化（深褐色系替换金色系）
- [x] 攻略完成卡片装饰系统（封章+角落+呼吸光效）
- [x] NPC 专属色双色映射（--npc-accent 自动切换）
- [x] 主题切换 CSS transition 过渡动画（0.35s ease，防止快速切换闪烁）
- [x] NPC 完成态强调色精修（color-mix 比例提升：border 65%, glow 45%, name 65%）
- [x] 日间模式卡片层次增强（border-color + box-shadow 覆写）
- [x] 布局紧凑化（content-area 自适应、卡片 66→46px、状态栏 68→50px）
- [x] 性能优化（dist 28.8→6.0 MB：图片压缩、移除 WebpackObfuscator、清理 14 个未使用依赖）

### 已知问题

- [x] 切换到夜间模式时字体显示异常 — 已修复（BUG-1: void document.documentElement.offsetHeight 强制同步重算）
- [x] 主题切换瞬间跳变 — 已修复（添加 0.35s CSS transition 过渡动画）

### 构建与部署

- [x] esbuild 独立脚本构建（var_structure.js + backend_validate.js）✅ 2026-05-21
- [x] 控制台零项目错误 ✅ 2026-05-21

### 待实施

- [x] 阶段三: 商城/背包/图鉴页面样式推广（2026-05-20 完成，金册玉牒视觉风格已铺开）
- [x] 阶段三补充: 图鉴 `因果秘录` 时间线归档（E-UI-21 完成，心音册 / 风声卷已具备因果轴与刻点）
- [x] 阶段四: 渐变中心优化 + 性能调优 + 响应式适配 ✅ 已完成（2026-05-21）
...
1. **渐变中心动态追踪** — `useGradientCenter.ts`
   - 监听 `.content-area` 的 scroll 事件
   - Y 轴渐变中心随滚动从 30% 移动到 70%
   - X 轴固定 50%

2. **响应式 media queries** — `_global.scss`
   - `<400px`：缩小太极图标(20px)、压缩间距、减小字号
   - `>600px`：放大太极图标(28px)、增大字号


---

## SystemBar V2: 仙俗沿满式重构 (2026-05-21)

### 设计理念

将功能栏从平面信示升级为「仙俗画卷」沿满式体验，采用三栏水墨布局：

`
【左侧】角色经绲  |  【中间】天时保试  |  【右侧】仙家财产
【水墨分隔线】           【水墨分隔线】
`
...
| 功能 | 描述 | 技术实现 |
|------|------|----------|
| 水墨分隔线 | 三栏之间清淡渡向渡分隔 | SVG linearGradient |
| 呼吸光晕 | 时层图标和文字每动光晕 | shichen-glow 4s |
| 云雾特效 | 时层下方3个楚团渡移动 | mist-drift 6s |
| 倒计时警警告 | ≤5日震颤动画+暗气暗红 | countdown-shake 0.4s |
| 灵石重设计 | 图标+楚文文字金色数字 | spirit-stone-row + gem PNG |
| NPC特性光色 | 未攻略各致主颜色光晕,已攻略渊红粉色 | data-npc + per-NPC filter |
| 三栏布局 | 左中右 flex自适应 | flex: 1/auto/1 |

### CSS动画清单

`scss
@keyframes shichen-glow { 0%, 100% { opacity: 0.85; } 50% { opacity: 1; filter: drop-shadow(0 0 6px); } }
@keyframes mist-drift { 0%, 100% { transform: translateX(0); } 50% { transform: translateX(8px); } }
@keyframes countdown-shake { 0%, 100% { transform: translateX(0); } 25% { transform: translateX(-1px); } 75% { transform: translateX(1px); } }
`
## v4 视觉与交互闭环权威补充（2026-05-22）

### 已落地的视觉方向

- 主页状态栏从信息面板升级为“五气朝元阵”：场景切换、时辰、灵石与 NPC 命轮共同构成横向气运界面。
- NPC 卡片完成状态语义化：前端不直接展示裸数值，以 `灵犀` / `蚀心` 与双环命轮表达进度。
- 心里话系统采用“心音残片”显影：只有 `soul_whisper.is_revealed === true` 且 `text` 非空时才显示，避免前端伪造剧情结果。
- 牝奴期、商城、背包均完成去现代化文案清理，玩家可见文本从“购买/拥有/暂无装备”等现代词转为宗门语境。

### 美学与 MVU 的边界

- 动效可以即时发生：命轮点击、心防反震、按钮颤动、库存归零消散都可以由 CSS/前端状态驱动。
- 剧情结果不能即时伪造：心音内容、反噬扣数、剧情解锁必须等下一楼层 AI 通过 MVU 变量回流。
- 若最新楼层 MVU 没有 `soul_whisper`，前端不显示“心音残片”是正确的沉默，而不是视觉缺陷。

### 实机经验

- 裸 JSON Patch 会出现在正文中，但不会稳定回流变量；美学层再完整也无法弥补变量未应用。
- 因此视觉验收必须同时看两件事：页面是否漂亮，以及 `Mvu.getMvuData(...latest)` 是否真的有对应变量。
- 场景文字与 MVU 场景名均允许开放扩展；界面色相与卡片过滤以 `系统.场景上下文.公开度`、`在场NPC` 和快捷入口 fallback 为锚点，不再依赖三枚举收敛。
### v4 设计验收红线

- 不得把“前端沉默”误判为视觉缺陷：如果最新 MVU 没有 `soul_whisper` 回流，心音区域就必须保持隐藏或空灵留白。
- 不得回退到现代组件语义：商城、背包、牝奴期、功能栏继续使用宗门语境，不恢复“购买/余额/暂无装备”等现代词。
- 不得回退到旧场景名：`喧闹大堂 / 微醺吧台 / 幽寂厢房` 已废弃；三核心地点仅是快捷入口，开放动态场景以 AI 回写的 `系统.场景上下文` 为准。
- 不得回退到错误术语：命魂盘使用“太极/命轮”，禁用“八卦”；阶段名使用“牝奴”，禁用“牡奴”。
