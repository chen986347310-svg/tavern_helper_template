...
- **场景锚点**：UI 场景只允许 `莲灯前苑`、`醉玉小筑`、`绮梦幽阁`；正文可写其他地点，但 MVU 字段必须映射回三大 UI 场景。
...
- 场景文字允许丰富描写，但 UI 场景必须收敛到 `莲灯前苑 / 醉玉小筑 / 绮梦幽阁` 三个主题场域，避免界面色相与卡片过滤失去锚点。
### v4 设计验收红线

- 不得把“前端沉默”误判为视觉缺陷：如果最新 MVU 没有 `soul_whisper` 回流，心音区域就必须保持隐藏或空灵留白。
- 不得回退到现代组件语义：商城、背包、牝奴期、功能栏继续使用宗门语境，不恢复“购买/余额/暂无装备”等现代词。
- 不得回退到旧场景名：`喧闹大堂 / 微醺吧台 / 幽寂厢房` 已废弃，文档与 UI 均以 `莲灯前苑 / 醉玉小筑 / 绮梦幽阁` 为准。
