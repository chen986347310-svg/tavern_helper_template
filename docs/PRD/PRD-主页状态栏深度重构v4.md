...
- **实机未通过项**：AI 仍输出 `变量更新情况` 与裸 JSON Patch，缺少 `<UpdateVariable>` / `<JSONPatch>` 包裹，导致 `NPC.白芷.soul_whisper` 未回流。
- **当前最高风险**：本地世界书规则已更新，但 SillyTavern 实际启用的角色卡/世界书可能仍是旧版本；`pnpm build` 只能证明前端状态栏已构建，不能证明世界书提示词已同步。
- **同步断点**：`package.json` 的 `sync` 指向 `node tavern_sync.mjs`，但当前工作区缺少 `tavern_sync.mjs`；必须先恢复或替代同步链路，再进行下一次实机验收。
- **场景约束**：AI 正文可以描写“外门广场”等地点，但写入 MVU 的 `系统.当前场景` / `NPC.*.当前场景` 必须映射回 `莲灯前苑`、`醉玉小筑`、`绮梦幽阁` 三个 UI 场景。
## 一、项目概述
...
待处理交互: z.array(z.object({
  类型: z.enum(['装备', '卸下', '购买', '灵识窃取', '装备道具', '购买物品']).prefault('购买'),
...
| 场景枚举漂移 | P1 | AI 写入 `外门广场` 等 schema 外场景 | 世界书规则要求叙事地点映射到三大 UI 场景 |
...
- **场景枚举验收**：写入 MVU 的 `系统.当前场景` 与 `NPC.*.当前场景` 只能是 `莲灯前苑`、`醉玉小筑`、`绮梦幽阁`；其他地点只能出现在正文叙事中。
...
- 新增风险：AI 在正文变量摘要中写入 `外门广场`，不属于当前 schema 枚举 `莲灯前苑 / 醉玉小筑 / 绮梦幽阁`，说明场景约束仍可能被旧提示或自由叙事冲淡。
